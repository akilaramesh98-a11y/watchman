...
function SecurityForm({ searchParams }: props) {
  ...
  const [checklists0, setChecklists0] = useState<ChecklistItem[]>([
    ...
    {
      point: '2. Does the vehicle have the required three tarpaulins for cargo protection, and are they in good condition and stitchless as per company guidelines ?',
      dropdown: '',
      images: {
        'Photo 1': '',
        'Photo 2': ''
  ...
  const [checklists3, setChecklists3] = useState<ChecklistItem[]>([
    ...
    // },
    {
      point: '3. What is the final loaded weight (in kgs) as shown in the provided image? Does the image include the Weighbridge Slip, if applicable ?',
      image: ''
    },
    {
      point: "4. Have all required documents been provided and verified ?",
      subItems: [
        {
  ...
  const [checklists4, setChecklists4] = useState<ChecklistItem[]>([
    ...
    {
      point: '4. Vehicle complete check for any irregular materials apart from Invoiced Ones is done and found irregular materials ?',
      image: '',
      dropdown: ''
    }
  ]);
  ...
  const handleSave = async () => {
    ...
    switch (currentStage) {
      ...
      case 5:
        ...
        payload.finished_at = dayjs().utc().format();
        break;
    }
    ...
    if (isValid) {
      ...
      if (currentStage === 1) {
        missingMandatoryFields = currentStageData.filter(item => {
          ...
          if (item.point === "2. Does the vehicle have the required three tarpaulins for cargo protection, and are they in good condition and stitchless as per company guidelines ?") {
            if (!item.dropdown) {
              return true;
            }
        ...
        missingMandatoryFields = currentStageData.filter((item, index) => {
          if (item.point === "4. Vehicle complete check for any irregular materials apart from Invoiced Ones is done and found irregular materials ?") {
            if (!item.dropdown) {
              return true; 
            }
            return false;
          ...
          if (item.point === "Point1" && item.inputValue === undefined) {
            ...
            if(item.inputValue.length < 10){
              return `${item.point} - Please enter at least one complete invoice number (10 digits)`;
            }
    ...
    switch (stage) {
      case 1:
        return stageData.every(item => {
          switch (item.point) {
            case "1. Is the vehicle body in satisfactory condition for transport ?":
              return !!item.dropdown && Object.values(item.images).every(img => !!img);
            case "2. Does the vehicle have the required three tarpaulins for cargo protection, and are they in good condition and stitchless as per company guidelines ?":
              return !!item.dropdown && Object.values(item.images).every(img => !!img);
            case "5. What is the verified carrying capacity of the vehicle in kilograms ?":
              return !!item.inputValue && !!item.image;
            case "6. What is the driver's license number and expiration date of the driver's license ?":
              return !!item.inputValue && !!item.date;
            case "7. Screening Driver to check wheather the person is under alocohol influence or not ?":
      ...
      case 3:
          return stageData.every(item => {
              switch (item.point) {
                  case "1. Has the vehicle body been re-inspected and confirmed to be in good condition ?":
                      return !!item.dropdown && Object.values(item.images).every(img => !!img);
                  case "2. Is the tarpaulin properly covering the entire bottom surface of the truck prior to loading ?":
                      return !!item.dropdown && Object.values(item.images).every(img => !!img);
                  case "3. Has the cross-check between Pick-Up Slip weight and Vehicle Passing weight been completed (in kgs) ?":
      ...
      case 4:
        return stageData.every(item => {
          switch (item.point) {
            case "1. Has the layer-wise loading and stuffing been executed according to the approved stacking plan ?":
              return !!item.dropdown && Object.values(item.images).every(img => !!img);
            case "2. Does the quantity of loaded material correspond precisely with the quantity listed in the Pick-Up List ?":
              return !!item.dropdown;
            // case "Has the truck been properly sealed, and if so, what is the Seal Number?":
            //   return !!item.dropdown && !!item.image;
            case "3. What is the final loaded weight (in kgs) as shown in the provided image? Does the image include the Weighbridge Slip, if applicable ?":
              return !!item.image;
            case "4. Have all required documents been provided and verified ?":
      ...
      case 5:
        return stageData.every(item => {
          switch (item.point) {
            case "1. Has the final truck sealing been completed, and what is the seal number, if applicable ?":
              return !!item.dropdown && !!item.image;
            case "2. What are the Commercial Invoice Numbers associated with this shipment ?":
              return !!item.inputValue;
            case "3. Have the e-way bill and invoice been verified to have valid and current dates ?":
              return !!item.dropdown;
            case "4. Vehicle complete check for any irregular materials apart from Invoiced Ones is done and found irregular materials ?":
              return !!item.dropdown;  
            default:
              return true;
          }
  ...
  useEffect(() => {
    const getVehicleData = async () => {
      ...
      if (data.statusCode === 200){
        ...
        const newSubmittedImages: {[key: string]: boolean} = {};
        ['stage1', 'stage2', 'stage3', 'stage4', 'stage5'].forEach((stage, stageIndex) => {
          if (security[stage]?.completed === true) {
  ...
  <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div className="bg-white p-6 rounded-lg shadow-lg w-[600px] relative">
      ...
      {(() => {
        ...
        if (currentChecklist && currentUploadIndex !== null && currentChecklist[currentUploadIndex]) {
          const currentItem = currentChecklist[currentUploadIndex]; 
          return (
      ...
      if (!currentItem) {
        console.error("Invalid item for upload index:", currentUploadIndex, "in checklist:", currentChecklist);
                ...
                <>
                  ...
                  <div className="col-span-2">
                    {currentChecklistIndex === 0 && currentUploadIndex === 2 || currentUploadIndex === 3 ? (
                      <LocalizationProvider dateAdapter={AdapterDayjs} locale={enIN}>
                      <DatePicker
                        label="Select Date"
                        ...
                        onChange={(newValue: dayjs.Dayjs | null) => {
                          if(newValue){
                            ...
                            const newChecklists = [...checklists0];
                            (newChecklists[currentUploadIndex] as any).date = utcDate.format();
                            setChecklists0(newChecklists);
                          }
                        }}
                        renderInput={(params) => (
                          <TextField
                            ...
                            onChange={(e) => {
                              ...
                              const parsedDate = dayjs(inputDate, 'DD/MM/YYYY', true);
                              if (parsedDate.isValid()) {
                                const newChecklists = [...checklists0];
                                (newChecklists[currentUploadIndex] as any).date = parsedDate.toDate();
                                setChecklists0(newChecklists);
                              }
                            }}
                            onBlur={(e) => {
                      ...
                      <TextField
                        label="Carrying capacity (kgs)"
                        value={(currentItem as any).inputValue || ''}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                          const newValue = e.target.value;
                          let validValue = newValue.replace(/[^0-9.]/g, '');

                          const dotIndex = validValue.indexOf('.');
                          if (dotIndex !== -1) {
                            validValue = validValue.slice(0, dotIndex + 1) + validValue.slice(dotIndex + 1).replace(/\./g, '');
                          }
                          if (validValue === '.') {
                            validValue = '';
                          }
                          ...
                          const newChecklists = [...checklists0];
                          (newChecklists[currentUploadIndex] as any).inputValue = newValue;
                          setChecklists0(newChecklists);
                        }}
                        onBlur={() => {
                          setInputError('')
                    ...
                    <Box display="flex" gap={2}>
                      <TextField
                        ...
                        onChange={(e) => {
                          ...
                          if (newValue !== validValue) {
                            setInputError("Only alphanumeric values are allowed.");
                          } else {
                            setInputError("");
                          }
                          const newChecklists = [...checklists0];
                          (newChecklists[currentUploadIndex] as any).inputValue = validValue;
                          setChecklists0(newChecklists);
                        }}
                        onBlur={() => {
                          setInputError('')
          ...
          //   );
          // } 
          if (currentChecklistIndex === 3 && currentUploadIndex === 2) {
            return (
              <ImageUploadSlot
        ...
        if (currentChecklistIndex === 0) {
          ...
          if (currentChecklistIndex === 0 && currentUploadIndex === 2 || currentUploadIndex === 3) {
            if (!currentItem.date) {
              ...
              const dateObject = dayjs.isDayjs(currentItem.date) ? currentItem.date : dayjs(currentItem.date);
          
              if (!dateObject.isValid()) {
                errors.push("Invalid date. Please enter a valid date.");
              } else {
                const currentDate = dayjs().startOf('day');
                if (dateObject.isBefore(currentDate)) {
                  errors.push("Date cannot be in the past. Please select a current or future date.");
                } else {
                  const formattedDate = dateObject.format('DD/MM/YYYY');
                  isNewDate = formattedDate !== currentItem.previousDate;
                }
              }
            }
            ...
            } else if (!/^(\d+\.?\d*|\.\d+)$/.test(currentItem.inputValue)) {
            ...
            if (currentItem.inputValue !== undefined) {
              ...
              } else if (/\./.test(currentItem.inputValue)) {
                errors.push("Dots are not allowed.");
              }  else if (/[^a-zA-Z0-9]/.test(currentItem.inputValue)) {
                errors.push("Only alphanumeric values are allowed.");
              } else {
                isNewInput = currentItem.inputValue !== currentItem.previousInputValue;
              }
            }
          ...
          if (currentChecklistIndex === 4 && currentUploadIndex === 0) {
            ...
            if (currentItem.inputValue !== undefined && currentItem.inputValue !== '') {      
              if (!/^(\d+\.?\d*|\.\d+)$/.test(currentItem.inputValue)) {
                errors.push("Only numeric values are allowed.");
              }  else {
                isNewInput = currentItem.inputValue !== currentItem.previousInputValue;
              } 
            ...
            if (!currentItem.inputValue) {
              errors.push("Please enter the required value.");
            } else if(currentItem.inputValue.length < 10){
              errors.push("Please enter at least one complete invoice number (10 digits)");
            } else {
              isNewInput = currentItem.inputValue !== currentItem.previousInputValue;
            }
          } 
                              ...
                              {item.subItems?.map((point:any, num) => (
                                <div key={num} className="flex flex-col space-y-2">
                                  ...
                                  <div className="flex flex-col space-y-2" >
                                    ...
                                    value="Yes" 
                                    // @ts-ignore
                                    checked={item?.subItems[num]?.dropdown === "Yes"}
                                    onChange={() => {
                                  ...
                                  <input 
                                    ...
                                    value="No" 
                                    // @ts-ignore
                                    checked={item?.subItems[num]?.dropdown === "No"}
                                    onChange={() => {
                                      if (!checklists3) return; 
                                      const newChecklists = [...checklists3];
                                      if(newChecklists.length > 0){
                                        ...
                                        {(item.subItems && item.subItems[num] && item.subItems[num].dropdown === "Yes") && (
                                          ...
                                          <input 
                                            ...
                                            onChange={(e) => {
                                              ...
                                              if (newValue !== validValue) {
                                                point.inputError = "Only numeric values are allowed.";
                                              } else {
                                                point.inputError = "";
                                              }
                                              ...
                                              if(newChecklists[index]?.subItems){
                                                // @ts-ignore
                                                newChecklists[index].subItems[num].inputValue = validValue;
                                              }
                                            ...
                                            onBlur={() => {
                                              ...
                                              if(newChecklists[index]?.subItems){
                                                // @ts-ignore
                                                newChecklists[index].subItems[num].inputError = "";
                                              }
                                              setChecklists3(newChecklists);
                                            }}
                                            placeholder="Enter details"
                                            className={`border rounded px-2 py-1 mt-2 ${point.inputError ? 'border-red-500' : ''}`}
                                            style={{
                                              fontSize: '12px',
                                              width: '200px',
                                            }}
                                          />
                                          {point.inputError && (
                                            <div className="text-red-500 text-xs mt-1">{point.inputError}</div>
                                          )}
                                      </>
                                        )}
                                      </div>
